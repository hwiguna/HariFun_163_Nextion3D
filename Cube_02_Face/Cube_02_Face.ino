...
uint16_t nVertices = sizeof(vertexes) / sizeof(Point);
...
uint16_t nFaces = sizeof(faces) / (4 * sizeof(uint8_t));
uint32_t colors[] = {NEX_COL_RED, NEX_COL_BROWN, NEX_COL_YELLOW, NEX_COL_WHITE, NEX_COL_BLUE , NEX_COL_GREEN};
...
void DrawCubeOLD(uint32_t kolor)
{
  //  //-- Draw vertices --
  //  for (int i = 0; i < nVertices; i++)
  //  {
...
{
  //== Compute vertices and screen coordinate versions of those vertices. ==
  Point vertexList[numVertices];
  Point screenCoords[numVertices];
  for (int vIndex = 0; vIndex < numVertices; vIndex++) {
    Point vertex = vertexes[vIndex];
    vertex.x -= cam.pos.x;
    vertex.y -= cam.pos.y;
    vertex.x -= cam.pos.x;
    ...
    vertexList[vIndex] = vertex;

    float f = (vertex.z == 0) ? y0 : (y0 / vertex.z); // Further (larger z) will result in smaller f, which makes the point closer to origin making it look smaller
    int x = x0 + vertex.x * f;
    int y = y0 + vertex.y * f;

    screenCoords[vIndex] = Point(x, y, 0);

    //== Draw Faces ==
    int face_list[numFaces];
    for (int faceIndex=0; faceIndex<numFaces; faceIndex++)
    {
      bool isOnScreen = false;
      for (int vertIndex=0; vertIndex<4; vertIndex++)
      {
        if (vert_list[vertIndex].z > 0) {
          isOnScreen = true;
          break;
        }
      }
